 ...
 * Exchange Token:
 *
 * Exchange the authentication response for the ID and access token
 */
...
app.post("/validate", async (req, res) => {
  ...
  const furtherValidations = (payload) => {
    ...
    //Validate that the issuer of the ID token (iss) is equal to one of the following, according to environment
  ...
  try {
    ...
    res.status(200).json({
      ok: true,
      msg: 'Your ID token has passed all of the validations described in the following guide: <a href="https://developer.bindid.io/docs/guides/integration_guide/topics/validateToken/validate_token" target="_blank">https://developer.bindid.io/docs/guides/integration_guide/topics/validateToken/validate_token</a>',
    });
  } catch (err) {
    res.status(401).json({
      ok: false,
